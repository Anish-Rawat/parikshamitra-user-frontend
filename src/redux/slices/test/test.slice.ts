...
import { API_URIS } from "@/utils/contant";
  ...
  export const createTest = createAsyncThunk(
    "create-test",
    async ({ accessToken, testName, difficultyLevel, totalQuestions, subjectId, classId }: { accessToken: string, testName: string, difficultyLevel: string, totalQuestions: number, subjectId: string, classId: string }) => {
      const createTestApiResponse = await fetch(
        `${process.env.NEXT_PUBLIC_DEV_BASE_URL}/${API_URIS.tests.createTest}`,
        {
          ...
          body: JSON.stringify({ testName, difficultyLevel, totalQuestions, subjectId, classId }),
        }
      );
